...
mod tests {
    ...
    fn test_basic_problem_with_status() {
        ...
        check!(StatusCode::BAD_REQUEST == problem.status);
        check!("tag:mundus,2021:some/problem" == problem.error.problem_type());
        check!(None == problem.detail);
        check!(None == problem.instance);
        check!(0 == problem.extra.len());
    }
    ...
    fn test_basic_problem() {
        ...
        check!(StatusCode::NO_CONTENT == problem.status);
        check!("tag:mundus,2021:some/problem" == problem.error.problem_type());
        check!(None == problem.detail);
        check!(None == problem.instance);
        check!(0 == problem.extra.len());
    ...
    fn test_full_problem() {
        ...
        check!(StatusCode::BAD_REQUEST == problem.status);
        check!("tag:mundus,2021:some/problem" == problem.error.problem_type());
        check!(Some("Some Detail".to_owned()) == problem.detail);
        check!(Some("Some Instance".to_owned()) == problem.instance);
        check!(2 == problem.extra.len());
        check!(Some(&serde_json::to_value("Some Value").unwrap()) == problem.extra.get(&"some_key".to_owned()));
        check!(Some(&serde_json::to_value(42).unwrap()) == problem.extra.get(&"other_key".to_owned()));
    }
}
